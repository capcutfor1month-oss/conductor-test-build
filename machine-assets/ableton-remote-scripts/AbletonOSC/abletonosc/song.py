...
class SongHandler(AbletonOSCHandler):
    ...
    def init_api(self):
        ...
        def master_get_panning(params: Tuple[Any] = ()):
            return (self.song.master_track.mixer_device.panning.value,)

        def master_set_panning(params: Tuple[Any] = ()):
            self.song.master_track.mixer_device.panning.value = float(params[0])
        ...
        self.osc_server.add_handler("/live/master_track/get/panning", master_get_panning)
        self.osc_server.add_handler("/live/master_track/set/panning", master_set_panning)

        #--------------------------------------------------------------------------------
        # Return tracks: volume, panning, name, num_return_tracks
        #--------------------------------------------------------------------------------
        def return_get_num(params: Tuple[Any] = ()):
            return (len(self.song.return_tracks),)

        def return_get_volume(params: Tuple[Any] = ()):
            idx = int(params[0])
            return (idx, self.song.return_tracks[idx].mixer_device.volume.value)

        def return_set_volume(params: Tuple[Any] = ()):
            idx = int(params[0])
            self.song.return_tracks[idx].mixer_device.volume.value = float(params[1])

        def return_get_panning(params: Tuple[Any] = ()):
            idx = int(params[0])
            return (idx, self.song.return_tracks[idx].mixer_device.panning.value)

        def return_set_panning(params: Tuple[Any] = ()):
            idx = int(params[0])
            self.song.return_tracks[idx].mixer_device.panning.value = float(params[1])

        def return_get_name(params: Tuple[Any] = ()):
            idx = int(params[0])
            return (idx, self.song.return_tracks[idx].name)

        def return_set_name(params: Tuple[Any] = ()):
            idx = int(params[0])
            self.song.return_tracks[idx].name = str(params[1])

        def return_get_mute(params: Tuple[Any] = ()):
            idx = int(params[0])
            return (idx, self.song.return_tracks[idx].mute)

        def return_set_mute(params: Tuple[Any] = ()):
            idx = int(params[0])
            self.song.return_tracks[idx].mute = bool(params[1])

        self.osc_server.add_handler("/live/song/get/num_return_tracks", return_get_num)
        self.osc_server.add_handler("/live/return_track/get/volume", return_get_volume)
        self.osc_server.add_handler("/live/return_track/set/volume", return_set_volume)
        self.osc_server.add_handler("/live/return_track/get/panning", return_get_panning)
        self.osc_server.add_handler("/live/return_track/set/panning", return_set_panning)
        self.osc_server.add_handler("/live/return_track/get/name", return_get_name)
        self.osc_server.add_handler("/live/return_track/set/name", return_set_name)
        self.osc_server.add_handler("/live/return_track/get/mute", return_get_mute)
        self.osc_server.add_handler("/live/return_track/set/mute", return_set_mute)
